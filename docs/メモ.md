...
- `findAllAsync(String)` でページ内のテキスト検索を開始
- `findNext(Boolean)` で次/前の検索結果に移動（`true`で次へ、`false`で前へ）
- `clearMatches()` で検索結果をクリア
- `setFindListener` で検索結果（マッチ数、現在位置、検索完了フラグ）をリアルタイムに取得可能

## 画像の長押し保存

- `WebView.setOnLongClickListener`で長押しイベントをハンドリング
- `WebView.hitTestResult`で長押しした要素の情報を取得
    - `hitTestResult.type`が`IMAGE_TYPE`または`SRC_IMAGE_ANCHOR_TYPE`の場合が画像
    - `hitTestResult.extra`で画像URLを取得可能
